    ...
    <script>
        ...
        let lastTrackId = "";
        let currentLyrics = [];
          let trackDurationMs = 0; // Stockage global de la durée totale
        ...
        async function redirectToSpotify() {
            ...
            const scope = "user-read-private user-read-email user-modify-playback-state user-read-playback-state user-library-read playlist-read-private";
 ...
 // --- ENVOI DE LA COMMANDE SEEK (NAVIGATION DANS LA BARRE) ---
    ...
    headers: { 'Authorization': 'Bearer ' + currentToken }
        ...
        async function fetchLyrics(artist, title, album, duration) {
            try {
                ...
                if (data.syncedLyrics) {
                    ...
                    document.getElementById('lyrics-content').innerText = data.plainLyrics || "Paroles synchronisées indisponibles.";
                    currentLyrics = [];
                }
            } catch (e) {
        ...
        /* NOUVEAU SYSTÈME DE COULEURS ÉCLATANTES */
        function updateDynamicBackground() {
            ...
            tempImg.onload = function() {
                try {
                    ...
                    // Si les couleurs extraites sont trop sombres (ex: pochette noire), on applique un boost
                    if ((r1 + g1 + b1) < 100) { r1 += 60; g1 += 40; b1 += 90; }
                    if ((r2 + g2 + b2) < 100) { r2 += 90; g2 += 50; b2 += 40; }

                    // On injecte les couleurs vives dans l'aurora floutée
        ...
        async function updateNowPlaying() {
            ...
            try {
                ...
                if (data && data.item) {
                      trackDurationMs = data.item.duration_ms; // Sauvegarde de la durée totale
                    ...
                    // Mise à jour de la barre de progression visuelle et des timers
